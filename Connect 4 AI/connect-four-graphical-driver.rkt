...
(define (main)
  ...
  (define-values (process-1 process-stdout-1 process-stdin-1 process-stderr-1)
    (apply subprocess #f #f stderr-1 exe-1 args-1))
  ...
  (define-values (process-2 process-stdout-2 process-stdin-2 process-stderr-2)
    (apply subprocess #f #f stderr-2 exe-2 args-2))
  ...
  (instantiate menu-item%
    ...
    (callback
     (lambda (menu-item event)
       ...
       (send player-1-exe-text-field set-value exe-1)
       (send player-1-args-text-field set-value (format "~s" args-1))
       (send player-2-exe-text-field set-value exe-2)
       (send player-2-args-text-field set-value (format "~s" args-2))
       (send dialog show #t)))))
  ...
  (instantiate button%
    ...
    (callback
     (lambda (button event)
       ...
       (set! args-1 (with-input-from-string (send player-1-args-text-field get-value) (lambda () (read))))
       (set! exe-2 (send player-2-exe-text-field get-value))
       (set! args-2 (with-input-from-string (send player-2-args-text-field get-value) (lambda () (read))))
